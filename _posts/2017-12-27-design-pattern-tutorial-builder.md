...
有人看到这个类图发现, 咦，乖乖, 怎么跟其他的类图不太一样呢? 确实不一样, 因为其他的类图太抽象了. 理解起来如果没有代码辅助的话, 对于初学者很难理解. 所以我就自己画了一个类图;
...
当然啦, 这个接口与产品的接口基本上一模一样, 确实是这样的, 毕竟产品的构建者也需要这些零件才能构建出来完整的产品呢;
...
那么, 如果我们需要增加一个属性值 age, 我们也只需要在User中增加一个age属性 与 get方法, 同时修改Builder类; 而上层调用仅仅只需要在build()调用前, 增加一个withAge()的调用即可. 虽然这种方式对于扩展并不是开放的, 但是尽可能的减少了由于增加字段而使得上层代码改动很大的情况.

而我上面举的例子正是源自于Effective Java书中的第二条建议: 可以通过建造者模式构建不可变对象。
